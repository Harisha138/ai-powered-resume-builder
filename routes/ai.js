...
router.post("/generate-summary", requireAuth, checkAICredits, async (req, res) => {
  try {
    ...
    The summary should be 2-3 sentences, highlight key achievements, and be ATS-friendly.`
...
router.post("/improve-description", requireAuth, checkAICredits, async (req, res) => {
  try {
    ...
    Please rewrite this to be more impactful, quantifiable, and achievement-focused. Use action verbs and include metrics where possible. Format as bullet points.`
...
router.post("/suggest-skills", requireAuth, checkAICredits, async (req, res) => {
  try {
    ...
    Focus on both technical and soft skills that are in high demand. Avoid duplicating current skills. 
